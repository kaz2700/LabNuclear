...
PLOTS_DIR = os.path.join(OUT_DIR, 'graficos')
...
# (Detector 1's CFTD provides the gate; Detector 0 records its energy in MCAch0)
...
# Also fit with scripts-style and Fortran for comparison
...
# Total counts and N_acc
# The 511+1274 coincidence produces TWO peaks in the MCAch0 spectrum:
# 1) 511 keV peak (det 0 sees 511, det 1 gates on 1274)
# 2) 1274 keV peak (det 0 sees 1274, det 1 gates on 511)
# N_(511+1274) = area(511 peak) + area(1274 peak)
N_total_157 = np.sum(cnts157)
N_511_1274_total = 0
if fit_511_1274:
    N_511_1274_total += fit_511_1274['area']
if fit_1274_157:
    N_511_1274_total += fit_1274_157['area']
N_acc_157 = N_total_157 - N_511_1274_total

print(f"N_total(157°) = {N_total_157:.0f}")
print(f"N_(511 peak from 511+1274) = {fit_511_1274['area']:.1f} ± {fit_511_1274['darea']:.1f}" if fit_511_1274 else "N_(511) = NO DISPONIBLE")
print(f"N_(1274 peak from 511+1274) = {fit_1274_157['area']:.1f} ± {fit_1274_157['darea']:.1f}" if fit_1274_157 else "N_(1274) = NO DISPONIBLE")
print(f"N_(511+1274) total = {N_511_1274_total:.1f}")
print(f"N_acc(157°) = N_total - N_(511+1274) = {N_acc_157:.1f}")
print()
...
$$N_{{511+1274}} = N_{{511}} + N_{{1274}} = {fit_511_1274['area']:.1f} + {fit_1274_157['area']:.1f} \\approx {N_511_1274_total:.1f}$$

$$N_{{acc}} = N_{{\\text{{total}}}} - N_{{511+1274}} = {N_total_157:.0f} - {N_511_1274_total:.1f} = {N_acc_157:.1f}$$
...
report += f"| $N_{{\\text{{511+1274}}}}$ total (157°) | {N_511_1274_total:.1f} |\n"
report += f"| $N_{{\\text{{total}}}}$ (157°) | {N_total_157:.0f} |\n"
report += f"| $N_{{\\text{{acc}}}}$ (157°) | {N_acc_157:.1f} |\n"
if fit_511_511_180:
...
report_path = os.path.join(OUT_DIR, 'informe_coincidencias.md')
...
import json
with open(os.path.join(OUT_DIR, 'resultados.json'), 'w') as f:
