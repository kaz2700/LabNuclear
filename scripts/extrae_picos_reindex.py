...
# Define peak windows with reindexed coordinates (x_start=1) for Fortran code
# Fortran hardcoded: X_GAUSS_START=9, X_GAUSS_END=21
peaks = [
    ('22Na-157-coin', '511_157', 66, 90),
    ('22Na-157-coin', '1274_157', 188, 210),
    ('22Na-180-coin', '511_180', 66, 92),
    ('22Na-180-coin', 'pico_200_180', 190, 215),
]

os.makedirs('data', exist_ok=True)
for fname, label, xmin, xmax in peaks:
    meta, ch, counts = cargar_espectro(fname)
    mask = (ch >= xmin) & (ch <= xmax)
    x = ch[mask]
    y = counts[mask]
    # Reindex x to start at 1 for the Fortran code
    x_reindex = np.arange(1, len(x) + 1, dtype=int)
    outpath = os.path.join('data', f'pico_{label}.dat')
    with open(outpath, 'w') as f:
        for xi, yi in zip(x_reindex, y):
            f.write(f'{xi} {int(yi)}\n')
    print(f'{outpath}: {len(x)} pts, ymax={y.max()}, x0_orig={xmin + (np.argmax(y))}')
