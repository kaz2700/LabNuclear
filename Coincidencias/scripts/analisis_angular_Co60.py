...
Ajuste gaussiano de picos: Fortran (src/gaussian_background.f)
Ajuste de Legendre W(θ): scipy (no es caracterización gaussiana)
"""
...
# ─── Fortran Gaussian fit for 1332 peak ─────────────────────────────────

def fit_peak_1332(ch, counts, fortran_bin=FORTRAN_BIN):
    est = 205
    window = 15
...
for i, (r, c) in enumerate(zip(results, colors)):
    ...
    if f is not None:
        x_fit = f['x_fit']
        bg_arr = f.get('a_bg', 0) + f.get('b_bg', 0) * (f.get('x0_re', 0) - f.get('shift', 0) + x_fit - x_fit[0])
        y_fit_model = f['A'] * np.exp(-(x_fit - f['x0'])**2 / (2 * f['sigma']**2)) + bg_arr
        ax.plot(x_fit, y_fit_model / r['live_time'], 'r-', lw=1.5)
    ax.set_xlim(120, 240)
...
summary = {
    'metodo_gaussiano': 'Fortran (src/gaussian_background.f)',
    'metodo_angular': 'scipy.curve_fit (Legendre)',
    'A2': float(A2_fit) if 'A2_fit' in dir() else None,
    'A4': float(A4_fit) if 'A4_fit' in dir() else None,
    'data': out_data,
}

with open(os.path.join(BASE_DIR, 'resultados', 'resultados_angular_ch1.json'), 'w') as f:
