...
FORTRAN_BIN = os.path.join(os.path.dirname(BASE_DIR), 'src', 'gaussian_background')
...
# Peak 511 keV from 511+1274 coinc
...
# 1274 keV peak from 511+1274 coinc
...
# Accidental counts: N_acc = N(511) - N(1274)
...
print(f"N_(511 peak from 511+1274) = {fit_511_1274['area']:.1f}" if fit_511_1274 else "N_(511) = NO DISPONIBLE")
print(f"N_(1274 peak from 511+1274) = {fit_1274_157['area']:.1f}" if fit_1274_157 else "N_(1274) = NO DISPONIBLE")
...
El sistema de adquisición registra en MCAch0 la energía depositada en el detector 0,
...
Por tanto, el espectro de MCAch0 a 157° contiene **dos picos**: uno a 511 keV y
...
En el espectro a 157°, las coincidencias reales 511+1274 producen igual número de cuentas
en el pico de 511 keV y en el de 1274 keV (es aleatorio qué detector ve cada fotón).
El exceso de cuentas en el pico de 511 keV sobre el de 1274 keV corresponde a
coincidencias accidentales 511+511 de dos desintegraciones distintas:

$$N_{{\\text{{acc}}}}(157°) = N_{{511}} - N_{{1274}} = {fit_511_1274['area']:.1f} - {fit_1274_157['area']:.1f} = {N_acc_157:.1f}$$

$N_{{acc}}$ incluye principalmente coincidencias accidentales 511+511 de dos
desintegraciones distintas, además de eventos de fondo Compton bajo los picos.
...
Cada detector capta uno de ellos. El espectro de MCAch0 muestra el pico de 511 keV
...
El sistema de adquisición (XIA Pixie4) registra en MCAch0 la energía del detector 0
cuando hay coincidencia. Dependiendo de qué detector ve qué energía:
- Detector 0 ve 511 keV y detector 1 ve 1274 keV → pico de 511 keV en MCAch0.
- Detector 0 ve 1274 keV y detector 1 ve 511 keV → pico de 1274 keV en MCAch0.
...
report += f"| $N_{{\\text{{511}}}}$ (157°, 511+1274) | {fit_511_1274['area']:.1f} |\n"
report += f"| $N_{{\\text{{1274}}}}$ (157°, 511+1274) | {fit_1274_157['area']:.1f} |\n"
report += f"| $N_{{\\text{{acc}}}} = N_{{511}} - N_{{1274}}$ (157°) | {N_acc_157:.1f} |\n"
if fit_511_511_180:
...
Código: `analisis_coincidencias.py`. Ajustes: Fortran (`src/gaussian_background.f`).*
"""

report_path = os.path.join(BASE_DIR, 'informe', 'informe_coincidencias.md')
...
resultados['areas']['N_511'] = fit_511_1274['area'] if fit_511_1274 else 0
resultados['areas']['N_1274'] = fit_1274_157['area'] if fit_1274_157 else 0
...
import json
with open(os.path.join(BASE_DIR, 'resultados', 'resultados.json'), 'w') as f:
...
print(f"  Informe:    informe/informe_coincidencias.md")
print(f"  Gráficos:   graficos/ch0/")
print("="*60)
